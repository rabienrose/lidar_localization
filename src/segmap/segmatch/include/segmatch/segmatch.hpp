...
class SegMatch {
 public:
  ...
  /// recognized.
  const PairwiseMatches& recognize(const PairwiseMatches& predicted_matches, Eigen::Matrix4f& g_pose);
  ...
  void setAsTargetCloud();

  std::vector<database::MergeEvent> getMergeEvents() { return merge_events_; }
  
  void get_target_matrix(Eigen::MatrixXf& target_matrix_, std::vector<unsigned int>& indices_list);
  void get_match_points( PointICloud& cloud, std::vector<unsigned int>& indices_list);
